...
public class SQLiteCursor {
	...
	public int intValue(int columnIndex) throws SQLiteException {
		...
		return columnIntValue(preparedStatement.getStatementHandle(),
				columnIndex);
	}
	...
	public double doubleValue(int columnIndex) throws SQLiteException {
		...
		return columnDoubleValue(preparedStatement.getStatementHandle(),
				columnIndex);
	}
	...
	public String stringValue(int columnIndex) throws SQLiteException {
		...
		return columnStringValue(preparedStatement.getStatementHandle(),
				columnIndex);
	}
	...
	public String columnName(int columnIndex) throws SQLiteException {
		...
		return columnName(preparedStatement.getStatementHandle(),
				columnIndex);
	}
	

	/**
	 * Возвращаемый ByteBuffer доступен только до перемещения курсора или
	 * повторного вызова запроса
	 * 
	 * @param columnIndex
	 * @return
	...
	public ByteBuffer byteBufferValue(int columnIndex) throws SQLiteException {
		...
		ByteBuffer buf = columnByteBufferValue(
				preparedStatement.getStatementHandle(), columnIndex);
		if (buf != null) {
	...
	public Object objectValue(int columnIndex) throws SQLiteException {
		...
		int type = columnType(preparedStatement.getStatementHandle(),
				columnIndex);
		switch (type) {
		case FIELD_TYPE_INT:
